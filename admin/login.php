...
//نام کاربری
  ...
  if (!isset($username) || $username == ''){
	 header('Location:index.php?message=فیلد نام کاربری نباید خالی باشد ');

    $check_error = 1;
}
//کلمه عبور
elseif (!isset($password) || $password == ''){
	 header('Location:index.php?message=فیلد رمزعبور نباید خالی باشد ');
...
if ($check_error != 1){
    //تطبیق اطلاعات کاربر با آنچه که در دیتابیس ذخیره شده
        print("<br/>".$check_error);
          $result = select ("SELECT * FROM link_admin WHERE admin_ident = '$username' AND admin_pass = '$password'");
...
print_r($user);
    ...
    if($count > 0){
...

        {
            ...
            ob_start();
header('Location:index_admin.php?message=با موفقیت وارد شدید');    //this line works now
            ob_end_flush();
        ...
        echo "اطلاعات وارد شده صحیح نیست!<br />";
				ob_start();
            header('Location:index.php?message=اطلاعات وارد شده صحیح نیست. ');    //this line works now
         ob_end_flush();
